...
#define __FILENAME__ \
    (strstr(__FILE__, "src") ? strstr(__FILE__, "src") + 3 : __FILE__)
...
#define LOGFMT(proj, section, msg...) \
    LLOG("[", BLUE_TEXT(proj, "/"), CYAN_TEXT(section), "]:\t", msg, "\n");
...
class Logger
{
    ...
    private:
    static inline void EnableFileOutput(
        std::string filepath = LOG_DEFAULT_FILE) {
        if(filepath.size() != 0)
            filePath = filepath;
